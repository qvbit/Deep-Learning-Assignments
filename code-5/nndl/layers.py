...
def batchnorm_forward(x, gamma, beta, bn_param):
  ...
  if mode == 'train':
    ...
    mu = np.mean(x, axis=0)
    a = x - mu
    var = np.mean(a**2, axis = 0)
    std = std = np.sqrt(var + eps)
    xh = a / std
    out = gamma * xh + beta
    
    cache = (mode, x, gamma, a, std, xh, out)
    
    #update running means and running vars
    ...
    xh = (x - running_mean) / std
    out = gamma * xh + beta
    
    cache = (mode, x, xh, gamma, beta, std)
    # ================================================================ #
    # END YOUR CODE HERE
    # ================================================================ #
...
def batchnorm_backward(dout, cache):
  ...
  mode, x, gamma, a, std, xh, out = cache
    
  N = x.shape[0]
  dbeta = np.sum(dout, axis = 0)
  dgamma = np.sum(xh * dout, axis = 0)
  dxh = dout * gamma
  da = dxh / std
  dmu = -(1/std) * np.sum(dxh, axis = 0)
  db = a * dxh
  dc = -(1/std) * db
  de = -0.5 *(1/std**3) * db
  dvar = np.sum(de, axis = 0)
    
  dx = da + (2*a)/N * dvar + (1/N)*dmu
