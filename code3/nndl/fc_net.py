...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None):
    ...
    h1, c1 = affine_relu_forward(X, W1, b1)
    scores, c_scores = affine_forward(h1, W2, b2)
    ...
    #Get loss, gradient of softmax
    dloss, dscores = softmax_loss(scores, y)
    #account for regularization
    rloss = 0.5*self.reg*np.sum(W1**2) + 0.5*self.reg*np.sum(W2**2)
    loss = dloss + rloss
    
    #Backdrop into the second layer
    dh1, dW2, db2 = affine_backward(dscores, c_scores)
    dW2 += self.reg * W2
    
    #backdrop into first layer
    dx, dW1, db1 = affine_relu_backward(dh1, c1)
    dW1 += self.reg * W1
...
class FullyConnectedNet(object):
  ...
  def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
    ...
    dims = [input_dim] + hidden_dims + [num_classes] #input_dim = 15, hidden_dims = [20, 30], num_classes = 10. Result: [10,20,30,15]
    for i in np.arange(self.num_layers): #num_layers = 3
  ...
  def loss(self, X, y=None):
    ...
    #now grads holds gradient for all layers (2 in this case) 
